...
import { ScrapedArticleData } from './types.js';
...
export class EvidenceScraper {
   ...
   */
  private parseEvidenceAlertsPage(html: string): ScrapedArticleData {
    const $ = cheerio.load(html);
    const result: ScrapedArticleData = {};
   ...
   */
  async testScraping(testUrl?: string): Promise<boolean> {
   ...
   */
  async scrapeMultipleArticles(urls: string[]): Promise<Map<string, ScrapedArticleData>> {
    const results = new Map<string, ScrapedArticleData>();
