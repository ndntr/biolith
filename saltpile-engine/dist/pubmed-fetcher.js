...
export class PubMedFetcher {
    constructor() {
        ...
        this.requestDelay = 100; // 100ms between requests (10/second with API key)
        this.maxRetries = 3;
     ...
     */
    async fetchArticleAbstract(params) {
        try {
     ...
     */
    async parseArticleXml(xmlData, pmid) {
        try {
            ...
            if (article.PubmedData && article.PubmedData.ArticleIdList) {
                const idList = Array.isArray(article.PubmedData.ArticleIdList.ArticleId)
                    ...
                    : [article.PubmedData.ArticleIdList.ArticleId];
                const doiObj = idList.find((id) => id.IdType === 'doi');
                if (doiObj && doiObj._) {
                    doi = doiObj._;
                }
            }
     ...
     */
    async fetchMultipleAbstracts(articles) {
